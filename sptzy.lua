...
MiniBtn.MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        ...
        for _, v in pairs(MainFrame:GetChildren()) do
            if v ~= MiniBtn and v ~= Border and v ~= UICorner_M then v.Visible = false end
        end
        MiniBtn.Text = "+"
        ...
        task.wait(0.2)
        for _, v in pairs(MainFrame:GetChildren()) do
            if v ~= MiniBtn then v.Visible = true end
        end
        MiniBtn.Text = "—"
        MiniBtn.Position = UDim2.new(1, -30, 0, 5)
    end
...
NameLabel.Size = UDim2.new(0, 110, 0, 15)
...
UserLabel.Size = UDim2.new(0, 110, 0, 12)
...
task.spawn(function()
    while true do
        for i = 0, 1, 0.01 do Border.BackgroundColor3 = Color3.fromHSV(i, 0.8, 1) task.wait(0.03) end
    end
end)
...
local function AddButton(name, icon, color, func)
    ...
    Btn.MouseButton1Click:Connect(function()
        act = not act
        Btn.BackgroundColor3 = act and color or Color3.fromRGB(30, 30, 30)
        func(act)
    end)
end
...
AddButton("MOBILE FREECAM", "rbxassetid://6034289542", Color3.fromRGB(0, 160, 80), function(s)
    freecamOn = s
